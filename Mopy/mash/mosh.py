...
class Progress:
    ...
    def setBaseScale(self,base=0.0,scale=1.0):
        if scale == 0: raise exception.ArgumentError(u'Scale must not equal zero!')
        self.base = base
        self.scale = scale
...
class Tes3Reader:
    ...
    def unpackSubHeader(self,recType='----',expName=None,expSize=0):
        ...
        if expName and expName != name:
            raise exception.FileError(self.inName,u'%s: Expected %s subrecord, but found %s instead.'
...
class SubRecord:
    ...
    def dump(self,out):
        if self.changed: raise exception.StateError(u'Data changed: '+ self.name)
        if not self.data: raise exception.StateError(u'Data undefined: '+self.name)
...
class Record:
    ...
    def getSize(self):
        if self.changed: raise exception.AbstractError
        return self.size
...
class ListRecord(Record):
    ...
    def __init__(self,name,size,delFlag,recFlag,ins=None,unpack=False):
        ...
        if name not in ('LEVC','LEVI'):
            raise exception.ArgumentError(u'Type must be either LEVC or LEVI.')
        #--Data
        self.id = None
    ...
    def loadData(self,ins):
        ...
        while bytesRead < self.size:
            ...
            #--Deleted?
                ...
                self.isDeleted = True
            #--Else
            else: raise exception.Tes3UnknownSubRecord(self.inName,name,self.name)
        #--No id?
        if not self.id:
            raise exception.FileError(self.inName,u'No id for %s record.' % (self.name,))
        #--Bad count?
    ...
    def loadData(self,ins):
        ...
        while bytesRead < self.size:
            ...
            #--Bad record?
                ...
                raise exception.FileError(self.inName,u'Extraneous subrecord (%s) in %s record.'
                    % (name,self.name))
        ...
        while bytesRead < self.size:
            ...
            if name == 'FRMR':
                ...
                #--New Record?
                    ...
                    if size != 4: raise exception.ModSizeError(self.inName,'CELL.FRMR',size,4,True)
            ...
            #--Map Note?
                ...
                if subGroup >= 20:
                    raise exception.FileError(self.ins, self.getId()+u': Second NAM0 subrecord.')
                subGroup = 20
                if size != 4: raise exception.ModSizeError(self.inName,'CELL.NAM0',size,4,True)
                if size != 4: raise exception.ModSizeError(self.inName,'CELL.NAM0',size,4,True)
...
class Dial(Record):
    ...
    def loadData(self,ins):
        ...
        if bytesRead != self.size:
            raise exception.FileError(self.inName,u'DIAL %d %s: Unexpected subrecords' % (self.type,self.id))
    ...
    def sortInfos(self):
        ...
        for info in self.infos:
            if info.id == None: raise exception.FileError(self.inName,u'Dialog %s: info with missing id.' % (self.id,))
    ...
    def loadData(self,ins):
        ...
        while bytesRead < self.size:
            ...
            #--Other subrecords
                ...
                raise exception.FileError(self.inName,u'Unknown SCPT record: '+name)
            bytesRead += 8+size
        if bytesRead != self.size:
            raise exception.FileError(self.inName,u'SCPT %d %s: Unexpected subrecords' % (self.type,self.id))

    def getRef(self):
        """Returns reference data for a global script."""
        rnam = self.rnam
        if not rnam or rnam.data == chr(255)*4: return None
        if rnam.size != 4: raise exception.FileError(self.inName,u'SCPT.RNAM',rnam.size,4,True)
...
class Tes3_Hedr(SubRecord):
    ...
    def getSize(self):
        if not self.data and not self.changed: raise exception.StateError(u'Data undefined: '+self.name)
        if not self.changed: return self.size
...
class Tes3(Record):
    ...
    def loadData(self,ins):
        ...
        while bytesRead < self.size:
            ...
            if size > MAX_SUB_SIZE: raise exception.ModSizeError(self.inName,name,size,-MAX_SUB_SIZE,True)
            #--Masters
            if name == 'MAST':
                #--FileName
...
class MWIniFile:
    ...
    def loadIni(self):
        ...
        while True:
            ...
            if not line:
                ins.close()
                raise exception.FileError('Morrowind.ini', u'Morrowind.ini: [GameFiles] section not found.')
    ...
    def save(self):
        ...
        if self.hasChanged(): raise exception.StateError(u'Morrowind.ini has changed')
...
class FileInfo:
    ...
    def getHeader(self):
        ...
        try:
            ...
            if name != 'TES3': raise exception.FileError(self.name,u'Expected TES3, but got '+name)
            self.tes3 = Tes3(name,size,delFlag,recFlag,ins,True)
        except struct.error, rex:
            ins.close()
            raise exception.FileError(self.name,u'Struct.error: '+`rex`)
...
class InstallerArchive(Installer):
    ...
    def refreshSource(self,archive,progress=None,fullRefresh=False):
        ...
        result = ins.close()
        if result:
            raise exception.InstallerArchiveError(u'Unable to read archive %s (exit:%s).' % (archive.s,result))
    ...
    def unpackToTemp(self,archive,fileNames,progress=None):
        ...
        if not fileNames: raise exception.ArgumentError(u'No files to extract for %s.' % (archive.s,))
        progress = progress or bolt.Progress()
        ...
        result = ins.close()
        if result:
            raise exception.StateError(u"Extraction failed.")
...
class FileRep:
    ...
    def save(self,outPath=None):
        ...
        if not self.canSave: raise exception.StateError(u'Insufficient data to write file.')
        if not outPath:
...
class FileRefs(FileRep):
    ...
    def refresh(self):
        ...
        if self.isDamaged:
            raise exception.StateError(self.fileInfo.name+u': Attempted to access damaged file.')
    ...
    def save(self,outPath=None):
        """Save data to file.
        outPath -- Path of the output file to write to. Defaults to original file path."""
        if not self.canSave or self.skipObjRecords: raise exception.StateError(u'Insufficient data to write file.')
        if not outPath:
            fileInfo = self.fileInfo
            outPath = os.path.join(fileInfo.dir,fileInfo.name)
        out = file(outPath,'wb')
    ...
    def getFirstObjectIndex(self):
        ...
        if not self.fileInfo.isEsp(): raise exception.StateError(u'FileRefs.renumberObjects is for esps only.')
        for cell in self.cells:
    ...
    def renumberObjects(self,first):
        ...
        if not self.fileInfo.isEsp(): raise exception.StateError(u'FileRefs.renumberObjects is for esps only.')
        if first <= 0: raise exception.ArgumentError(u'First index should be a positive integer')
        log = self.log
        next = int(first)
        for cell in self.cells:
            objects = cell.getObjects()
...
class WorldRefs:
            ...
            for object in masterCell.getObjects().list():
                ...
                if iMMod:
                    if iMMod >= len(masterMap):
                        raise exception.Tes3RefError(masterName,cellId,objId,iObj,iMMod,
                            _('NO SUCH MASTER'))
    ...
    def getMasterMap(self,masterInfo):
        ...
        for mmName in masterInfo.masterNames:
            if mmName not in self.masterNames:
                raise exception.BoltError(u"Misordered esm: %s should load before %s" % (mmName, masterInfo.name))
    ...
    def removeDebrisCells(self,fileRefs):
        ...
        if not fileRefs.fileInfo.isEss():
            ...
            raise exception.ArgumentError(u'Cannot remove debris cells from a non-save game!'+fileName)
        log = self.log
        cntDebrisCells = 0
    ...
    def removeDebrisRecords(self,fileRefs):
        ...
        if not fileRefs.fileInfo.isEss():
            fileName = fileRefs.fileInfo.fileName
            raise exception.ArgumentError(u'Cannot remove save debris from a non-save game!'+fileName)
        goodRecords = []
        debrisIds = self.debrisIds
        debrisTypes = set(debrisIds.keys())
    ...
    def removeOverLists(self,fileRefs):
        ...
        if not fileRefs.fileInfo.isEss():
            fileName = fileRefs.fileInfo.fileName
            raise exception.ArgumentError(u'Cannot remove overriding lists from a non-save game!'+fileName)
    ...
    def loadText(self,fileName):
        ...
        for line in inn:
            ...
            if not stripped:
                ...
                raise exception.BoltError(u'Bad line in CharSet class file.'+line.strip()+u' >> '+stripped)
        inn.close()
        #--Post Parse
        for className,stats in self.classStats.items():
            if len(stats) != 35:
                raise exception.BoltError(u'Bad number of stats for class '+className)
...
class ScheduleGenerator:
            ...
            #--Data
                ...
                if section == 'import':
                    ...
                    if not (newPath and os.path.exists(newPath)):
                        raise exception.StateError(u'Unable to import schedule file: '+line.strip())
                ...
                #--Evening/Night
                    ...
                    for chunk in chunks[1:]:
                        ...
                        if not maSleep or maSleep.group(1) == '=':
                            raise exception.BoltError(u'Bad sleep condition state for %s in %s: %s'
                                % (section,town,line))
                        condition,state = maSleep.group(2), sleepStates[maSleep.group(1)]
                        condition = reIsMember.sub(ur'getPCRank \1 >= 0',condition)
                ...
                #--NPC
                    ...
                    if not maCell:
                        raise exception.BoltError(u'Pos cell not defined for %s %s %d' % (town,npc,cycle))
                    cell = maCell.group(1)
    ...
    def getResetScript(self):
        ...
        if not self.project: raise exception.StateError(u'No project has been defined!')
