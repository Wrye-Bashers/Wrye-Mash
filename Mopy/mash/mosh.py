...
class Progress:
    ...
    def __call__(self, rawProgress, message=None):
        ...
        if ((message != self.message) or
            ...
            self.doProgress(self.base + self.scale * rawProgress / self.max,
                message)
...
class Book(Record):
    ...
    def __init__(self, name='BOOK', size=0, delFlag=0, recFlag=0, ins=None,
        unpack=False):
        """Initialization."""
    ...
    def loadData(self, ins):
        ...
        while bytesRead < self.size:
            ...
            if name == 'NAME':
                ...
                (self.weight, self.value, self.isScroll, self.teaches,
                self.enchantPoints
...
class Cell(Record):
    ...
    def __init__(self, name='CELL', size=0, delFlag=0, recFlag=0, ins=None,
        unpack=False, skipObjRecords=False):
        # --Arrays
    ...
    def loadData(self, ins):
        ...
        while bytesRead < self.size:
            ...
            # --Start subrecord?
                ...
                if name == 'DATA':
                    (self.flags, self.gridX, self.gridY) = struct.unpack('3i',
                        record.data)
                bytesRead += 8 + size
                # print ' ',name,size
            # --Object sub-record?
    ...
    def dumpData(self, out):
        ...
        for record in self.records:
            if record.name == 'DATA':
                record.setData(
                    struct.pack('3i', self.flags, self.gridX, self.gridY))
            record.getSize()
            record.dump(out)
        # --Objects
        inTempObjects = False
...
class Crec(ContentRecord):
    ...
    def __init__(self, name='CREC', size=0, delFlag=0, recFlag=0, ins=None,
        unpack=False):
        # --Arrays
        self.id = None
        self.index = 0
...
class Cntc(ContentRecord):
    ...
    def __init__(self, name='CNTC', size=0, delFlag=0, recFlag=0, ins=None,
        unpack=False):
        # --Arrays
        self.id = None
        self.index = 0
...
class Dial(Record):
    ...
    def __init__(self, name='DIAL', size=0, delFlag=0, recFlag=0, ins=None,
        unpack=False):
        # --Arrays
        self.id = None
        self.type = 0
...
class Fmap(Record):
    ...
    def __init__(self, name='FMAP', size=0, delFlag=0, recFlag=0, ins=None,
        unpack=False):
        """Initialize."""
            ...
            for yc in range(wcell):
                ...
                for xc in range(wcell):
                    ...
                    if height >= 0:  # --Land
                        ...
                        66, 48, 33, 32, 23, 16, sqrt(height / 3000.0))
                        scale = int(
                            scale * 10) / 10.0  # --Make boundaries sharper.
                        r = chr(max(0, int(r0 - r1 * scale)) & ~1)
                    else:  # --Sea
                        # --Scale color from shallow to deep color.
                        (r0, g0, b0, r1, g1, b1, scale) = (
                        37, 55, 50, 12, 19, 17, -height / 2048.0)
                        r = chr(max(0, int(r0 - r1 * scale)) | 1)
...
class Glob(Record):
    ...
    def __init__(self, name='GLOB', size=0, delFlag=0, recFlag=0, ins=None,
        unpack=False):
        """Initialization."""
        self.type = 'l'
        self.value = 0
...
class Info_Test:
    ...
    def dumpData(self, out, index):
        ...
        # --SCVR
        out.pack('4siBB2sB',
            'SCVR', 5 + len(self.text), index + 48, self.type, self.func,
            self.oper)
        if self.text:
            out.write(self.text)
        # --Value
...
class Info(Record):
    ...
    def __init__(self, name='INFO', size=0, delFlag=0, recFlag=0, ins=None,
        unpack=False):
        """Initialization."""
        # --Info Id
        self.id = ''
    ...
    def loadData(self, ins):
        ...
        while bytesRead < self.size:
            ...
            # --String/Value Tests
                ...
                (self.type, self.spDisp, self.spRank, self.spSex, self.pcRank,
                self.unk02
...
class Land(Record):
    ...
    def __init__(self, name='LAND', size=0, delFlag=0, recFlag=0, ins=None,
        unpack=False):
        """Initialize."""
        self.id = None
        # self.gridX = 0
...
class Npcc(ContentRecord):
    ...
    def __init__(self, name='NPCC', size=0, delFlag=0, recFlag=0, ins=None,
        unpack=False):
        # --Arrays
        self.id = None
        self.index = 0
...
class Scpt(Record):
    """SCPT record. Script."""
    # --Class Data
    subRecordNames = ['SCVR', 'SCDT', 'SCTX', 'SLCS', 'SLSD', 'SLFD', 'SLLD',
        'RNAM']

    def __init__(self, name='SCPT', size=0, delFlag=0, recFlag=0, ins=None,
        unpack=False):
        # --Arrays
        self.id = None
        self.numShorts = 0
    ...
    def loadData(self, ins):
        ...
        while bytesRead < self.size:
            ...
            # --Header
            if name == 'SCHD':
                (self.id, self.numShorts, self.numLongs, self.numFloats,
    ...
    def dumpData(self, out):
        ...
        out.packSub('SCHD', '32s5i',
            ...
            self.dataSize, self.varSize)
        # --Others
        for record in [getattr(self, srName.lower(), None) for srName in
            Scpt.subRecordNames]:
            if not record:
                continue
...
class Tes3_Hedr(SubRecord):
    ...
    def getSize(self):
        ...
        if not self.changed:
            return self.size
        self.description = winNewLines(self.description).encode('utf-8')
        self.author = winNewLines(self.author).encode('utf-8')
...
class Tes3(Record):
    ...
    def __init__(self, name='TES3', size=0, delFlag=0, recFlag=0, ins=None,
        unpack=False):
        """Initialize."""
        self.hedr = None
    ...
    def getStats(self):
        ...
        while not ins.atEnd():
            ...
            if type not in stats:
                ...
                stats[type] = (
                count + 1, cumSize + size + 16)  # --16B in header
            # --Seek to next record
...
class ModInfos(FileInfos):
    ...
    def circularMasters(self, stack, masters=None):
        stackTop = stack[-1]
        masters = masters or (
        stackTop in self.data and self.data[stackTop].masterNames)
        if not masters:
            return False
        for master in masters:
            if master in stack:
    ...
    def getLoadOrder(self, modNames, asTuple=True):
        ...
        modNames.sort()
        modNames.sort(
            key=lambda a: (a in data) and data[a].mtime)  # --Sort on modified
    ...
    def unload(self, fileName, doSave=True):
        ...
        for loadFile in loadFiles:
            ...
            if not mwIniFile.isLoaded(loadFile):
                continue
            if loadFile not in self.data:
                continue  # --Can happen if user does an external delete.
        ...
        for subrecord in self.tes3.others:
            if subrecord.name == 'SCRS':
                ...
                for num in xrange(len(subrecord.data) / 4):
                    bb, gg, rr = struct.unpack('3B',
                        subrecord.data[num * 4:num * 4 + 3])
                ...
                rgbSqAvg = float(sum(xx * xx for xx in rgbArray)) / len(
                    rgbArray)
...
class ScreensData(DataDict):
    ...
    def refresh(self):
        ...
        newData = {}
        reImageExt = re.compile(ur'\.(bmp|jpg)$',re.I|re.U)
        # --Loop over files in directory
        for fileName in self.dir.list():
            ...
            if maImageExt and filePath.isfile():
                newData[fileName] = (
                maImageExt.group(1).lower(), filePath.mtime)
        changed = (self.data != newData)
        self.data = newData
        return changed
...
class Installer(object):
    ...
    'dirty_sizeCrc',
    'comments', 'readMe', 'packageDoc', 'packagePic', 'src_sizeCrcDate',
    'hasExtraData',
    'skipVoices', 'espmNots')
    volatile = (
    'data_sizeCrc', 'skipExtFiles', 'skipDirFiles', 'status', 'missingFiles',
    ...
    dataDirs = set((
    'bookart', 'docs', 'fonts', 'icons', 'meshes', 'music', 'shaders', 'sound',
    'splash', 'textures', 'video', 'mash plus', 'mits'))
    ...
    docExts = set((
    '.txt', '.rtf', '.htm', '.html', '.doc', '.odt', '.jpg', '.png', '.pdf',
    '.css', '.xls'))
    # --Temp Files/Dirs
    ...
    def refreshDataSizeCrc(self):
        ...
        for full, size, crc in self.fileSizeCrcs:
            ...
            if (rootLower == 'omod conversion data' or
                    fileLower[-9:] == 'thumbs.db' or fileLower[
            -11:] == 'desktop.ini'):
        ...
        if self.isActive and data_sizeCrc != old_sizeCrc:
            ...
            for file, sizeCrc in old_sizeCrc.iteritems():
                if file not in dirty_sizeCrc and sizeCrc != data_sizeCrc.get(
                    file):
    ...
    def refreshBasic(self, archive, progress=None, fullRefresh=False):
        ...
        # --SubNames, SubActives
        if type == 2:
            actives = set(x for x, y in zip(self.subNames, self.subActives) if
            (y or x == ''))
            self.subNames = sorted(subNameSet, key=string.lower)
            if len(
                self.subNames) == 2:  # --If only one subinstall, then make it active.
...
class InstallerArchive(Installer):
    ...
    def install(self, archive, destFiles, data_sizeCrcDate, progress=None):
        ...
        self.unpackToTemp(archive, dest_src.values(),
...
class InstallersData(bolt.TankData, DataDict):
    ...
    def getSorted(self, column, reverse):
        ...
        if column == 'Package':
            ...
            if column in ('Package', 'Group'):
                getter = lambda x: object.__getattribute__(data[x],
                    attr).lower()
    ...
    def getGuiKeys(self, item):
        ...
        # --Icon
        iconKey = ('off', 'on')[installer.isActive] + '.' + self.status_color[
            installer.status]
        if installer.type < 0:
            iconKey = 'corrupt'
    ...
    def uninstall(self, unArchives, progress=None):
        ...
        for archive in sorted(data, key=getArchiveOrder, reverse=True):
            ...
            if archive in unArchives:
                for data_sizeCrc in (
                installer.data_sizeCrc, installer.dirty_sizeCrc):
                    for file, sizeCrc in data_sizeCrc.iteritems():
                        sizeCrcDate = data_sizeCrcDate.get(file)
                        if file not in masked and sizeCrcDate and sizeCrcDate[
                        :2] == sizeCrc:
                            removes.add(file)
            ...
            #  And/or may block later uninstalls.
                ...
                for file in myRestores:
                    if installer.data_sizeCrc[file] != data_sizeCrcDate.get(
                        file, (0, 0, 0))[:2]:
        ...
        if ['bash.installers.autoAnneal'] and restoreArchives:
            ...
            for index, archive in enumerate(restoreArchives):
                ...
                installer = data[archive]
                destFiles = set(
                    x for x, y in restores.iteritems() if y == archive)
                if destFiles:
        ...
        * Install missing files from active anPackages."""
        ...
        if restoreArchives:
            ...
            for index, package in enumerate(restoreArchives):
                ...
                installer = data[package]
                destFiles = set(
                    x for x, y in restores.iteritems() if y == package)
                if destFiles:
...
class FileRep:
    ...
    def save(self, outPath=None):
        ...
        self.tes3.hedr.numRecords = len(
            self.records)  # --numRecords AFTER TES3 record
        self.tes3.getSize()
        self.tes3.dump(out)
        # --Other Records
        for record in self.records:
...
class FileRefs(FileRep):
    ...
    def __init__(self, fileInfo, skipNonCells=False, skipObjRecords=False,
    ...
    def load(self):
        ...
        while not ins.atEnd():
            ...
            # --CELL?
            if name == 'CELL':
                record = Cell(name, size, delFlag, recFlag, ins, 0,
                    skipObjRecords)
                cells.append(record)
                if canSave:
                ...
                for object in objects.list():
                    ...
                    # --Remapped object?
                        ...
                        objIdBase = reObjNum.sub('',
                            objId)  # --Strip '00001234' id num from object
    ...
    def removeOrphanContents(self):
        ...
        self.records = [record for record in self.records if
            record not in orphans]
        self.conts = [record for record in self.conts if record not in orphans]
        self.conts_id = dict(
            [(id, record) for id, record in self.conts_id.iteritems() if
                matched[id] > 0])
        return len(orphans)
    ...
    def replaceRefsById(self, refReplacer):
        ...
        if newRecords:
            selfIds = set([record.getId().lower() for record in self.records if
                record.getId()])
...
class WorldRefs:
    ...
    def addMasters(self, masterNames):
        ...
        for masterName in masterNames:
            ...
            if not fileRefs:
                fileRefs = masterInfo.extras['FileRefs'] = FileRefs(masterInfo,
                    True, True)
    ...
    def addMaster(self, masterName):
        ...
        for levList in (
            masterRefs.debrisIds['LEVC'] + masterRefs.debrisIds['LEVI']):
    ...
    def removeDebrisRecords(self, fileRefs):
        ...
        for record in fileRefs.records:
            ...
            if type in debrisTypes:
                id = record.getId()
                if id and id.lower() not in debrisIds[
                    type] and not reCustomId.match(id):
    ...
    def repair(self, fileRefs):
        ...
        for cell in fileRefs.cells:
            ...
            if not isMod:
                ...
                if not (cell.flags & 1) and cellName and (
                    cellName not in self.extCellNames):
                    log(_("Debris Cell Name: ") + cellName)
                    cell.flags &= ~32
                    cell.cellName = ''
                    cell.setChanged()
                    cntUnnamed += 1
            refIds = cellRefIds.get(cellId,
                emptyDict)  # --Empty if cell is new in fileRefs.
            objects = cell.getObjects()
            for object in objects.list():
                (iMod, iObj, objId, objRecords) = object[:4]
                refKey = (iMod, iObj)
                ...
                for object in badObjects:
                    ...
                    if newMod and ((newMod, iObj) not in usedKeys):
                        ...
                        objects.replace(object,
                            fileRefs.remapObject(object, newMod))
                        cntRepaired += 1
                    elif not newMod:
    ...
    def repairWorldMap(self, fileRefs, gridLines=True):
        ...
        for gridx in range(-28, 28, 1):
            for gridy in range(28, -28, -1):
                ...
                fileRefs.fmap.drawCell(self.lands.get(id), gridx, gridy,
                    isMarked)
                proCount += 1
                progress(proCount)
...
class FileDials(FileRep):
    ...
    def loadText(self, textFileName):
        ...
        for line in textFile:
            ...
            if maInfo:
                infoKey = (
                int(maInfo.group(1)), maInfo.group(2), maInfo.group(3))
                texts[infoKey] = text = []
            else:
                text.append(line)
        textFile.close()
        ...
        # --Report
        buff = cStringIO.StringIO()
        for header, infoKeys in (
        (_('Updated'), updated), (_('Unmatched'), unmatched)):
            if infoKeys:
                buff.write('=== %s\n' % (header,))
...
class FileLibrary(FileRep):
    ...
    def loadUI(self,
        factory={'GLOB': Glob, 'BOOK': Book, 'SCPT': Scpt, 'CELL': Cell}):
    ...
    def copyBooks(self):
        ...
        for id, (record, modName) in (
            self.srcBooks.items() + self.altBooks.items()):
    ...
    def genLibData(self):
        ...
        for libId in self.libList:
            ...
            scriptCode = tsMain.substitute(
                libId=libId, srcId=srcId, ifAltId=(
                    (altId and tsIfAltId.substitute(libId=libId,
                        altId=altId)) or ''))
        ...
        # --Set scripts
        for id, code in (
        ('lib_setAllGS', setAllCode), ('lib_setNoneGS', setNoneCode)):
            code += ';--Done\nstopScript %s\nend\n' % (id,)
...
class FileLists(FileRep):
    ...
    def mergeWith(self, newFL):
        ...
        for levls, newLevls in (
        (self.levcs, newFL.levcs), (self.levis, newFL.levis)):
...
class FileScripts(FileRep):
    ...
    def loadText(self, textFileName):
        ...
        id_records = dict(
            (record.id.lower(), record) for record in self.scripts)
...
class CharSetImporter:
    ...
    def loadText(self, fileName):
        ...
        statNames = (
        'Agility', 'Block', 'Light Armor', 'Marksman', 'Sneak', 'Endurance',
        'Heavy Armor', 'Medium Armor', 'Spear', 'Intelligence', 'Alchemy',
        'Conjuration', 'Enchant', 'Security', 'Personality', 'Illusion',
        'Mercantile', 'Speechcraft', 'Speed', 'Athletics', 'Hand To Hand',
        'Short Blade', 'Unarmored', 'Strength', 'Acrobatics', 'Armorer', 'Axe',
        'Blunt Weapon', 'Long Blade', 'Willpower', 'Alteration', 'Destruction',
    ...
    def printMajors(self):
        ...
        for className, stats in sorted(self.classStats.items()):
            print className, '-------------------------------'
            skillStats = [(key, value) for key, value in stats.items() if
                key in skills]
...
class ScheduleGenerator:
    ...
    def loadText(self, fileName, pickScheduleFile=None, imported=None):
        ...
        for line in ins:
            ...
            if line[0] == '@':
                ...
                # --Non-npc?
                if id in set(
                    ['town', 'defs', 'night', 'evening', 'code', 'import',
                        'project']):
                    section = id
            ...
            # --Data
                ...
                # --Code
                    ...
                    if maCodeCycle:
                        codeCycles = [int(x) for x in
                            maCodeCycle.group(1).split(',')]
                        continue
                ...
                # --Evening/Night
                    ...
                    if not maSleep:
                        continue
                    (cell, defaultState) = (
                    maSleep.group(2), sleepStates[maSleep.group(1)])
                    ...
                    for chunk in chunks[1:]:
                        ...
                        condition, state = maSleep.group(2), sleepStates[
                            maSleep.group(1)]
                        condition = reIsMember.sub(r'getPCRank \1 >= 0',
                            condition)
                ...
                # --NPC
                    ...
                    coords[-1] = `int(
                        coords[-1]) * 57`  # --Workaround interior rotation bug
    ...
    def save(self, fileInfo):
        ...
        for town in sorted(self.newTowns):
            # --Cycle Scripts
            for cycle in (1, 2, 3, 4):
                setScript('SC_%s_%d' % (town, cycle),
                    self.getCycleScript(town, cycle))
            # --Master, sleep scripts
            for cycle in (3, 4):
                setScript('SC_%s_C%d' % (town, cycle),
                    self.getSleepScript(town, cycle))
            setScript('SC_%s_Master' % (town,), self.getMasterScript(town))
        # --Reset Scripts
        if self.project:
            setScript('SC_%s_ResetGS' % (self.project,), self.getResetScript())
            setScript('SC_%s_ResetStatesGS' % (self.project,),
                self.getResetStatesScript())
        # --Add dialog scripts
        # --Find Hello entries
        recIndex = 0
    ...
    def getSleepScript(self, town, cycle):
        ...
        if len(tcSleep) == 0:
            ...
            for cell in sorted(tcSleep.keys(), key=lambda a: a[1:-1],
                reverse=True):
    ...
    def getCycleScript(self, town, cycle):
        ...
        if townCode:
            for line in townCode[0] + townCode[cycle]:
                cycleCode += '\t' + line + '\n'
        out.write(self.tsCycle1.substitute(town=town, cycle=`cycle`,
            cycleCode=cycleCode))
        for npc in npcs:
            out.write('if ( "%s"->getDisabled )\n' % (npc,))
            out.write(
                'elseif ( "%s"->getItemCount SC_offSchedule != 0 )\n' % (npc,))
            for (condition, npcSchedule) in townSchedule[npc]:
                ...
                if ai != 'NOAI':
                    ...
                    if maWanderCell and (int(maWanderCell.group(1)) > 0) and (
                        maPosCell.group(4).find(',') == -1):
