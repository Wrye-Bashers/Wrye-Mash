...
# TES3 File Errors ------------------------------------------------------------
# class Tes3Error same as FileError
# class Tes3ReadError same as ModReadError
# class Tes3SizeError same as ModSizeError
class Tes3RefError(FileError):
    # Bolt Error handles message
    # FileError handles in_name and message
    ...
    def __init__(self, inName, cellId, objId, iObj, iMod, masterName=''):
        ...
        message = '{!s}: Bad Ref: {!s}: objId: {!s} iObj: {:d}'.format(inName,
            cellId, objId, iObj)
        if iMod:
            message += ' iMod: {:d} {!s}'.format(iMod, masterName)
        super(Tes3RefError, self).__init__(inName, message)


class Tes3UnknownSubRecord(FileError):
    """TES3 Error: Unknown subrecord."""

    # Bolt Error handles message
    # FileError handles in_name and message
    def __init__(self, inName, subName, recName):
        message = 'Extraneous subrecord {!s} in {!s} record.'.format(subName,
            recName)
        super(Tes3UnknownSubRecord, self).__init__(inName, message)


class MaxLoadedError(BoltError):
    """Usage Error: Attempt to add a mod to load list when load list is full."""

    def __init__(self, message=u'Load list is full.'):
        super(MaxLoadedError, self).__init__(message)


class InterfaceError(BoltError): pass
