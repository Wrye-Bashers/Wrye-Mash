...
class HTMLHelpParser(file):
    ...
    def Parse(self):
        ...
        # head
        head = self.raw_data[
        self.raw_data.lower().find("<head>"):self.raw_data.lower().find(
            "</head>") + len("</head>")]
        # body
        body = self.raw_data[
        self.raw_data.lower().find("<body>"):self.raw_data.lower().find(
            "</body>") + len("</body>")]
        # table of content
        start = body.lower().find(
            """<a name="tableofcontent"></a>""") and body.lower().find(
            """<a name='tableofcontent'></a>""")
        end = min(len(body), (body.lower().find(
            """<a name="tableofcontent_end"></a>""") and body.lower().find(
            """<a name='tableofcontent_end'></a>""")) + (len(
            """<a name="tableofcontent_end"></a>""") and len(
            """<a name='tableofcontent_end'></a>""")))
        self.content = preHead + "<html>" + head + "<body>" + body[
        start:end] + "</body></html>"
        # pages
        rawPages = body[body.lower().find("""<a name="pages"></a>""") + len(
            """<a name="pages"></a>"""):body.lower().find(
            """<a name="pages_end"></a>""")]
        # parse the hrefs of the table of content and retrieve only the internal doc refs
        hrefIndex = self.content.lower().find(
            """<p class=list-1>&bull;&nbsp; <a href="#""")
        if hrefIndex == -1:
            raise "Empty TOC"
        endPrevPage = 0
        # build up the pages
        self.pages = {}
        while hrefIndex != -1:
            hrefEnd = self.content.find(">", hrefIndex + len(
                """<p class=list-1>&bull;&nbsp; <a href="#""") - 2)
            pageRef = self.content[hrefIndex + len(
                """<p class=list-1>&bull;&nbsp; <a href="#""") - 2:hrefEnd]
            pageName = pageRef.replace("#", "")
            _rawPages = rawPages[endPrevPage:]
            pageIndex = _rawPages.find(pageName.strip(""" " ' """))
            hrefIndex = self.content.lower().find(
                """<p class=list-1>&bull;&nbsp; <a href="#""", hrefEnd)
            pageBegin = _rawPages.lower().find("<h2>")
            pageEnd = _rawPages[pageBegin + 4:].lower().find("<h2>")
            if pageEnd != -1:
                pageEnd = _rawPages[pageBegin + 4:].lower().find(
                    "<h2>") + pageBegin + 4
            endPrevPage += pageEnd
            page = _rawPages[pageBegin:pageEnd]
            # correcting images addresses
            if "<img " in page:
                page = page.replace("""<img src="images/""",
                    """<img src="%s\\""" % os.path.join(os.getcwd(), "images"))
            # add the header and footer to the page data
            self.pages[pageName] = HTMLHelpPage(
                preHead + "<html>" + head + "<body>" + page + "</body></html>")
    ...
    def GetPage(self, name):
        ...
        if n in self.pages.keys():
            ...
            for p, v in self.pages.iteritems():
                if (v.read().find("name=%s" % n) != -1 or v.read().find(
                        "name=%s" % N) != -1) == True:
