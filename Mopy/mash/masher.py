...
installercons.data.extend({
    # --Off/Archive
    'off.green'     : Image(os.path.join(imgPath, ur'checkbox_green_off.png'),
        wx.BITMAP_TYPE_PNG),
    'off.grey'      : Image(os.path.join(imgPath, ur'checkbox_grey_off.png'),
        wx.BITMAP_TYPE_PNG),
    'off.red'       : Image(os.path.join(imgPath, ur'checkbox_red_off.png'),
        wx.BITMAP_TYPE_PNG),
    'off.white'     : Image(os.path.join(imgPath, ur'checkbox_white_off.png'),
        wx.BITMAP_TYPE_PNG),
    'off.orange'    : Image(os.path.join(imgPath, ur'checkbox_orange_off.png'),
        wx.BITMAP_TYPE_PNG),
    'off.yellow'    : Image(os.path.join(imgPath, ur'checkbox_yellow_off.png'),
        wx.BITMAP_TYPE_PNG),
    # --On/Archive
    'on.green'      : Image(os.path.join(imgPath, ur'checkbox_green_inc.png'),
        wx.BITMAP_TYPE_PNG),
    'on.grey'       : Image(os.path.join(imgPath, ur'checkbox_grey_inc.png'),
        wx.BITMAP_TYPE_PNG),
    'on.red'        : Image(os.path.join(imgPath, ur'checkbox_red_inc.png'),
        wx.BITMAP_TYPE_PNG),
    'on.white'      : Image(os.path.join(imgPath, ur'checkbox_white_inc.png'),
        wx.BITMAP_TYPE_PNG),
    'on.orange'     : Image(os.path.join(imgPath, ur'checkbox_orange_inc.png'),
        wx.BITMAP_TYPE_PNG),
    'on.yellow'     : Image(os.path.join(imgPath, ur'checkbox_yellow_inc.png'),
        wx.BITMAP_TYPE_PNG),
    # --Off/Directory
    'off.green.dir' : Image(os.path.join(imgPath, ur'diamond_green_off.png'),
        wx.BITMAP_TYPE_PNG),
    'off.grey.dir'  : Image(os.path.join(imgPath, ur'diamond_grey_off.png'),
        wx.BITMAP_TYPE_PNG),
    'off.red.dir'   : Image(os.path.join(imgPath, ur'diamond_red_off.png'),
        wx.BITMAP_TYPE_PNG),
    'off.white.dir' : Image(os.path.join(imgPath, ur'diamond_white_off.png'),
        wx.BITMAP_TYPE_PNG),
    'off.orange.dir': Image(os.path.join(imgPath, ur'diamond_orange_off.png'),
        wx.BITMAP_TYPE_PNG),
    'off.yellow.dir': Image(os.path.join(imgPath, ur'diamond_yellow_off.png'),
        wx.BITMAP_TYPE_PNG),
    # --On/Directory
    'on.green.dir'  : Image(os.path.join(imgPath, ur'diamond_green_inc.png'),
        wx.BITMAP_TYPE_PNG),
    'on.grey.dir'   : Image(os.path.join(imgPath, ur'diamond_grey_inc.png'),
        wx.BITMAP_TYPE_PNG),
    'on.red.dir'    : Image(os.path.join(imgPath, ur'diamond_red_inc.png'),
        wx.BITMAP_TYPE_PNG),
    'on.white.dir'  : Image(os.path.join(imgPath, ur'diamond_white_inc.png'),
        wx.BITMAP_TYPE_PNG),
    'on.orange.dir' : Image(os.path.join(imgPath, ur'diamond_orange_inc.png'),
        wx.BITMAP_TYPE_PNG),
    'on.yellow.dir' : Image(os.path.join(imgPath, ur'diamond_yellow_inc.png'),
        wx.BITMAP_TYPE_PNG),
    # --Broken
    'corrupt'       : Image(os.path.join(imgPath, ur'red_x.png'),
        wx.BITMAP_TYPE_PNG),
}.items())
...
class SashTankPanel(gui.NotebookPanel):
    ...
    def __init__(self, data, parent):
        ...
        self.left = leftSash(self, defaultSize=(sashPos, 100),
...
class ListEditorDialog(wx.Dialog):
    ...
    def __init__(self, parent, id, title, data, type='list'):
        ...
        # --Done
        if data.__class__ in mosh.settings['mash.window.sizes']:
            self.SetSizer(sizer)
            self.SetSize(
                mosh.settings['mash.window.sizes'][data.__class__])
        else:
    ...
    def OnCloseWindow(self, event):
        ...
        sizes = mosh.settings.getChanged('mash.window.sizes')
...
class MasterList(guiWxList):
    ...
    def __init__(self, parent, fileInfo):
        self.parent = parent
        # --Columns
        self.cols = mosh.settings['mash.masters.cols']
        self.colNames = mosh.settings['mash.colNames']
        self.colWidths = mosh.settings['mash.masters.colWidths']
        self.colAligns = mosh.settings['mash.masters.colAligns']
        self.colReverse = mosh.settings['mash.masters.colReverse'].copy()
        # --Data/Items
        self.edited = False
        ...
        self.sort = mosh.settings['mash.masters.sort']
        self.esmsFirst = mosh.settings['mash.masters.esmsFirst']
        self.selectedFirst = mosh.settings['mash.masters.selectedFirst']
        # --Links
    ...
    def OnLabelEdited(self, event):
        ...
        if newName in mosh.modInfos:
            ...
            mosh.settings.getChanged('mash.mods.renames')[
    ...
    def SetFileInfo(self, fileInfo):
        ...
        self.newMasters.extend(
            mosh.modInfos.getLoadOrder(self.oldMasters, False))
    ...
    def SortItems(self, col=None, reverse=-2):
        ...
        if col == 'File':
            ...
            self.items.sort(lambda a, b: cmp(self.GetMasterStatus(a),
                self.GetMasterStatus(b)))
        elif col == 'Author':
        ...
        if reverse:
            self.items.reverse()
        # --ESMs First?
        mosh.settings['mash.masters.esmsFirst'] = self.esmsFirst
    ...
    def InitEdit(self):
        ...
        for itemId in self.items:
            ...
            if not masterInfo.modInfo:
                masterName = masterInfo.name
                newName = mosh.settings['mash.mods.renames'].get(masterName,
                    None)
                # --Rename?
    ...
    def OnColumnResize(self, event):
        ...
        mosh.settings.setChanged('mash.masters.colWidths')
...
class ModList(guiWxList, gui.ListDragDropMixin):
    ...
    def __init__(self, parent):
        # --Columns
        self.cols = mosh.settings['mash.mods.cols']
        self.colAligns = mosh.settings['mash.mods.colAligns']
        self.colNames = mosh.settings['mash.colNames']
        self.colReverse = mosh.settings.getChanged('mash.mods.colReverse')
        self.colWidths = mosh.settings['mash.mods.colWidths']
        # --Data/Items
        self.data = data = mosh.modInfos
        self.details = None  # --Set by panel
        self.sort = mosh.settings['mash.mods.sort']
        self.esmsFirst = mosh.settings['mash.mods.esmsFirst']
        self.selectedFirst = mosh.settings['mash.mods.selectedFirst']
        # --Links
    ...
    def PopulateItem(self, itemDex, mode=0, selected=set()):
        ...
        if fileName in selected:
            self.list.SetItemState(itemDex, wx.LIST_STATE_SELECTED,
                wx.LIST_STATE_SELECTED)
        else:
    ...
    def SortItems(self, col=None, reverse=-2):
        (col, reverse) = self.GetSortSettings(col, reverse)
        mosh.settings['mash.mods.sort'] = col
        ...
        if reverse:
            self.items.reverse()
        # --ESMs First?
        mosh.settings['mash.mods.esmsFirst'] = self.esmsFirst
        if self.esmsFirst or col == 'Load Order':
            self.items.sort(lambda a, b: cmp(a[-4:].lower(), b[-4:].lower()))
        # --Selected First?
        mosh.settings['mash.mods.selectedFirst'] = self.selectedFirst
    ...
    def OnDoubleClick(self, event):
        ...
        if not globalvars.docBrowser:
            DocBrowser().Show()
            mosh.settings['mash.modDocs.show'] = True
    ...
    def OnColumnResize(self, event):
        ...
        colName = self.cols[colDex]
        self.colWidths[colName] = self.list.GetColumnWidth(colDex)
        mosh.settings.setChanged('mash.mods.colWidths')
    ...
    def OnDrop(self, names, toIdx):
        ...
        if mosh.settings['mash.mods.sort'] != 'Modified':
    ...
    def moveSelected(self, event, moveMod):
        ...
        Changed in: Improved mod reordering code
...
class ModDetails(wx.Window):
    ...
    def OnSave(self, event):
        ...
        if changeName:
            ...
            mosh.settings.getChanged('mash.mods.renames')[oldName] = newName
...
class SaveList(guiWxList):
    ...
    def __init__(self, parent):
        # --Columns
        self.cols = mosh.settings['mash.saves.cols']
        self.colAligns = mosh.settings['mash.saves.colAligns']
        self.colNames = mosh.settings['mash.colNames']
        self.colReverse = mosh.settings.getChanged('mash.saves.colReverse')
        self.colWidths = mosh.settings['mash.saves.colWidths']
        # --Data/Items
        self.data = data = mosh.saveInfos
        self.details = None  # --Set by panel
        self.sort = mosh.settings['mash.saves.sort']
        # --Links
    ...
    def SortItems(self, col=None, reverse=-2):
        (col, reverse) = self.GetSortSettings(col, reverse)
        mosh.settings['mash.saves.sort'] = col
        data = self.data
        # --Start with sort by name
        ...
        if col == 'File':
            ...
            self.items.sort(
                lambda a, b: cmp(data[a].getStatus(), data[b].getStatus()))
        elif col == 'Player':
            self.items.sort(lambda a, b: cmp(
    ...
    def OnColumnResize(self, event):
        ...
        colName = self.cols[colDex]
        self.colWidths[colName] = self.list.GetColumnWidth(colDex)
        mosh.settings.setChanged('mash.saves.colWidths')
...
class SaveDetails(wx.Window):
    ...
    def OnBrowser(self, event):
        ...
        if not globalvars.journalBrowser:
            JournalBrowser().Show()
            mosh.settings['mash.journal.show'] = True
        if self.saveInfo:
    ...
    def OnChar(self, event):
        ...
        wx.WXK_UP, wx.WXK_DOWN):
            ...
            if event.GetKeyCode() == wx.WXK_DOWN:
                moveMod = 1
                visibleIndex = self.GetIndex(
                    sorted(self.GetSelected(), key=orderKey)[-1]) + 2
            else:
                moveMod = -1
                visibleIndex = self.GetIndex(
                    sorted(self.GetSelected(), key=orderKey)[0]) - 2
            for thisFile in sorted(self.GetSelected(), key=orderKey,
                reverse=(moveMod != -1)):
...
class InstallersPanel(SashTankPanel):
    ...
    def __init__(self, parent):
        ...
        # --Package
        self.gPackage = wx.TextCtrl(right, -1,
        ...
        self.gNotebook.SetSelection(
            mosh.settings['bash.installers.page'])
    ...
    def OnShow(self):
        """Panel is shown. Update self.data."""
        if mosh.settings.get('bash.installers.isFirstRun', True):
            mosh.settings['bash.installers.isFirstRun'] = False
            ...
            mosh.settings['bash.installers.enabled'] = balt.askYes(self,
                fill(message, 80), self.data.title)
        if not mosh.settings['bash.installers.enabled']:
            return
        if self.refreshing:
            return
    ...
    def SaveDetails(self):
        ...
        mosh.settings['bash.installers.page'] = self.gNotebook.GetSelection()
    ...
    def RefreshInfoPage(self, index, installer):
        ...
        if pageName == 'gGeneral':
            ...
            gPage.SetValue(
                dumpFiles(installer.mismatchedFiles, sNone, isPath=True))
...
class MashNotebook(wx.Notebook):
    def __init__(self, parent, id):
        ...
        # --Selection
        pageIndex = mosh.settings['mash.page']
        if mosh.settings['bash.installers.fastStart'] and pageIndex == 1:
            pageIndex = 2
...
class MashFrame(wx.Frame):
    ...
    def __init__(self, parent=None, pos=wx.DefaultPosition, size=(400, 500),
        ...
        wx.Frame.__init__(self, parent, wx.ID_ANY, u'Wrye Mash', pos, size,
            style)
        minSize = mosh.settings['mash.frameSize.min']
    ...
    def SetTitle(self, title=None):
        ...
        if not title:
            title = u"Wrye Mash {!s}".format(
                mosh.settings['mash.readme'][1])
            if 'mash.profile' in mosh.settings:
                title += u': ' + mosh.settings['mash.profile']
            wx.Frame.SetTitle(self, title)

    def SetStatusCount(self):
        """Sets the status bar count field. Actual work is done by current panel."""
        if hasattr(self,
            'notebook'):  # --Hack to get around problem with screens tab.
    ...
    def OnCloseWindow(self, event):
        ...
        if not self.IsIconized() and not self.IsMaximized():
            mosh.settings['mash.framePos'] = self.GetPosition()
            mosh.settings['mash.frameSize'] = self.GetSizeTuple()
        mosh.settings['mash.page'] = self.notebook.GetSelection()
    ...
    def CleanSettings(self):
        ...
        for fileInfos in (mosh.modInfos, mosh.saveInfos):
            ...
            backupDir = os.path.join(fileInfos.dir,
                mosh.settings['mosh.fileInfo.backupDir'])
...
class DocBrowser(wx.Frame):
    ...
    def __init__(self, modName=None):
        ...
        # --Window
        pos = mosh.settings['mash.modDocs.pos']
        size = mosh.settings['mash.modDocs.size']
        ...
        self.modNameList = wx.ListBox(self, -1,
        ...
        # --Doc display
        self.plainText = wx.TextCtrl(self, -1,
            style=wx.TE_READONLY | wx.TE_MULTILINE | wx.TE_RICH2)
        self.htmlText = wx.lib.iewin.IEHtmlWindow(self, -1,
    ...
    def DoSet(self, event):
        ...
        if modName in self.data:
            ...
            docsDir = (mosh.settings['mash.modDocs.dir'] or
                       os.path.join(mosh.settings['mwDir'], u'Data Files'))
            fileName = ''
        # --Dialog
        ...
        path = dialog.GetPath()
        dialog.Destroy()
        mosh.settings['mash.modDocs.dir'] = os.path.split(path)[0]
        if modName not in self.data:
    ...
    def DoRename(self, event):
        ...
        # --Remember change
        mosh.settings['mosh.workDir'] = os.path.split(path)[0]
        self.data[modName] = path
        self.SetMod(modName)
    ...
    def OnCloseWindow(self, event):
        ...
        Remember window size, position, etc."""
        self.DoSave()
        mosh.settings['mash.modDocs.show'] = False
        if not self.IsIconized() and not self.IsMaximized():
            mosh.settings['mash.modDocs.pos'] = self.GetPosition()
            mosh.settings['mash.modDocs.size'] = self.GetSizeTuple()
        self.Destroy()
...
class JournalBrowser(wx.Frame):
    ...
    def __init__(self, saveName=None):
        ...
        # --Window
        pos = mosh.settings['mash.journal.pos']
        size = mosh.settings['mash.journal.size']
    ...
    def OnCloseWindow(self, event):
        """Handle window close event.
        Remember window size, position, etc."""
        mosh.settings['mash.journal.show'] = False
        if not self.IsIconized() and not self.IsMaximized():
            mosh.settings['mash.journal.pos'] = self.GetPosition()
            mosh.settings['mash.journal.size'] = self.GetSizeTuple()
        self.Destroy()
...
class MashApp(wx.App):
    ...
    def OnInit(self):
        ...
        # --MWFrame
        frame = MashFrame(
            pos=mosh.settings['mash.framePos'],
            size=mosh.settings['mash.frameSize'])
        ...
        if mosh.settings['mash.modDocs.show']:
            DocBrowser().Show()
        if mosh.settings['mash.journal.show']:
            JournalBrowser().Show()
        if mosh.settings.get('mash.help.show'):
            HelpBrowser(globalvars.mashFrame, globalvars.images).Show()
        # -# D.C.-G. for SettingsWindow
        if mosh.settings['mash.settings.show']:
            globalvars.settingsWindow = SettingsWindow()
            globalvars.settingsWindow.SetSettings(mosh.settings,
    ...
    def SetMWDir(self):
        ...
        if os.path.exists(os.path.join(parentDir, u'Morrowind.ini')):
            mosh.settings['mwDir'] = parentDir
            mosh.dirs['app'] = GPath(parentDir)
            return True
        # --Already set?
        if os.path.exists(os.path.join(mosh.settings['mwDir'], u'Morrowind.ini')):
            return True
    ...
    def InitVersion(self):
        ...
        version = mosh.settings['mash.version']
        # --Version 0.42: MTimes from settings to ModInfos.table.
        if version < 42:
            mtimeKey = 'mosh.modInfos.mtimes'
            if mtimeKey in mosh.settings:
                modCol = mosh.modInfos.table.getColumn('mtime')
                for key, value in mosh.settings[mtimeKey].items():
                    modCol[key] = value[0]
                del mosh.settings[mtimeKey]
        ...
        if version < 60:
            ...
            for fileName in colGenre.keys():
                ...
                print fileName
            if mosh.settings['mash.mods.sort'] == 'Genre':
                mosh.settings['mash.mods.sort'] = 'Group'
            colWidths = mosh.settings['mash.mods.colWidths']
            if 'Genre' in colWidths:
                colWidths['Group'] = colWidths['Genre']
                del colWidths['Genre']
                mosh.settings.setChanged('mash.mods.colWidths')
        # --Version 0.71: Convert refRemoversdata to tuples
        if version < 71 and 'mash.refRemovers.data' in mosh.settings:
            import types
            data = mosh.settings['mash.refRemovers.data']
            for remover, path in data.items():
                if isinstance(path, types.StringTypes):
                    data[remover] = (path,)
            mosh.settings.setChanged('mash.refRemovers.data')
        # --Current version
        mosh.settings['mash.version'] = 71
        # --Version from readme
        readme = GPath(mosh.settings['mwDir']).join(u'Mopy', u'Wrye Mash.txt')
        if readme.exists() and readme.mtime != mosh.settings['mash.readme'][0]:
            reVersion = re.compile(ur"^=== ([\.\d]+) \[")
            for line in readme.open():
                maVersion = reVersion.match(line)
                if maVersion:
                    mosh.settings['mash.readme'] = (
...
class Files_SortBy(Link):
    ...
    def __init__(self, sortCol, prefix=''):
        ...
        self.sortName = mosh.settings['mash.colNames'][sortCol]
        self.prefix = prefix
...
class Files_Unhide(Link):
    ...
    def Execute(self, event):
        ...
        srcDir = os.path.join(destDir, mosh.settings['mosh.fileInfo.hiddenDir'])
        if self.type == 'mod':
        ...
        for srcPath in srcPaths:
            ...
            if os.path.exists(destPath):
                gui.dialog.WarningMessage(self.window,
                    _("File skipped: %s. File is already present.")
                    % (srcFileName,))
            # --Move it?
            else:
...
class File_Hide(Link):
    ...
    def Execute(self, event):
        """Handle menu selection."""
        message = _(
            r'Hide these files? Note that hidden files are simply moved to the Mash\Hidden subdirectory.')
        if gui.dialog.ContinueQuery(self.window, message, 'mash.hideFiles.continue',
            _('Hide Files')) != wx.ID_OK:
            return
        # --Do it
        destRoot = os.path.join(self.window.data.dir,
            mosh.settings['mosh.fileInfo.hiddenDir'])
            ...
            if not self.window.data.moveIsSafe(fileName, destDir):
                message = (_(
                    'A file named %s already exists in the hidden files directory. Overwrite it?')
                           % (fileName,))
                if gui.dialog.WarningQuery(self.window, message,
                    _('Hide Files')) != wx.ID_YES:
                    continue
...
class File_Sort(Link):
    ...
    def Execute(self, event):
        ...
        if gui.dialog.ContinueQuery(self.window, message, 'mash.sortMods.continue',
            _('Sort Mods')) != wx.ID_OK:
            return
...
class File_Snapshot(Link):
    ...
    def Execute(self, event):
        ...
        fileVersion = mosh.getMatch(re.search(r'[ _]+v?([\.0-9]+)$', fileRoot),
            1)
        snapVersion = mosh.getMatch(re.search(r'-[0-9\.]+$', destRoot))
        fileHedr = fileInfo.tes3.hedr
        if (fileVersion or snapVersion) and mosh.reVersion.search(
            fileHedr.description):
            newDescription = mosh.reVersion.sub(
                r'\1 ' + fileVersion + snapVersion,
...
class File_RevertToSnapshot(Link):
    ...
    def Execute(self, event):
        ...
        # --Snapshot finder
        destDir = self.window.data.dir
        srcDir = os.path.join(destDir, mosh.settings['mosh.fileInfo.snapshotDir'])
        ...
        message = (_("Revert %s to snapshot %s dated %s?")
                   % (fileInfo.name, snapName,
        formatDate(mosh.getmtime(snapPath))))
...
class File_RevertToBackup:
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        self.backup = os.path.join(self.fileInfo.dir,
            mosh.settings['mosh.fileInfo.backupDir'], self.fileInfo.name)
        menuItem.Enable(singleSelect and os.path.exists(self.backup))
        # --First Backup item
        wx.EVT_MENU(window, ID_REVERT_FIRST, self.Execute)
        menuItem = wx.MenuItem(menu, ID_REVERT_FIRST,
            _('Revert to First Backup'))
        menu.AppendItem(menuItem)
...
class File_Remove_RefsSafeCells(ListEditorData):
    ...
    def __init__(self, parent):
        """Initialize."""
        self.data = mosh.settings['mash.refRemovers.safeCells']
    ...
    def add(self):
        """Adds a safe cell."""
        # --Dialog
        dialog = wx.TextEntryDialog(self.parent, _('Cell Name:'),
            _('Add Safe Cell'))
        result = dialog.ShowModal()
        # --Canceled or empty?
        ...
        if newCell in self.data:
            return None
        mosh.settings.setChanged('mash.refRemovers.safeCells')
...
class File_Remove_RefsData(ListEditorData):
    ...
    def __init__(self, parent):
        """Initialize."""
        self.data = mosh.settings['mash.refRemovers.data']
        # --GUI
        ListEditorData.__init__(self, parent)
        self.showAdd = True
    ...
    def add(self):
        ...
        # --File dialog
        workDir = mosh.settings.get('mosh.workDir', mosh.settings['mwDir'])
        ...
        if len(paths) == 0:
            ...
            while (root % (number,)) in self.data:
                number += 1
            name = root % (number,)
        mosh.settings['mosh.workDir'] = os.path.split(paths[0])[0]
...
class File_Remove_Refs:
    ...
    def __init__(self):
        """Initialize."""
        self.safeCells = mosh.settings['mash.refRemovers.safeCells']
        self.removers = mosh.settings['mash.refRemovers.data']
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        wx.EVT_MENU_RANGE(window, ID_REMOVERS.BASE, ID_REMOVERS.MAX,
            self.DoList)
    ...
    def DoList(self, event):
        ...
        if gui.dialog.ContinueQuery(self.window, message, 'mash.refRemovers.continue',
...
class File_Replace_RefsData(ListEditorData):
    ...
    def __init__(self, parent):
        """Initialize."""
        self.data = mosh.settings['mash.refReplacers.data']
        # --GUI
        ListEditorData.__init__(self, parent)
        self.showAdd = True
        self.showRename = True
    ...
    def add(self):
        ...
        dialog.Destroy()
        mosh.settings['mosh.workDir'] = os.path.split(path)[0]
        mosh.settings.setChanged('mash.refReplacers.data')
...
class File_Replace_Refs:
    ...
    def __init__(self):
        """Initialize."""
        self.replacers = mosh.settings['mash.refReplacers.data']
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        wx.EVT_MENU_RANGE(window, ID_REPLACERS.BASE, ID_REPLACERS.MAX,
            self.DoList)
    ...
    def DoList(self, event):
        ...
        if gui.dialog.ContinueQuery(self.window, message, 'mash.refReplacers.continue',
            _('Replace Refs by Id...')) != wx.ID_OK:
            return
        # --File Refs
        fileName = self.data[0]
        fileInfo = self.window.data[fileName]
        ...
        try:
            ...
            if srcModName and srcModName not in mosh.modInfos:
                renames = mosh.settings['mash.mods.renames']
            ...
            try:
                ...
                message = ((
                               ...
                               _(
                                   "\n  Cell: %s\n  Object Id: %s\n  Object Index: %d") +
                               _("\n  Mod Index: %d (%s)")) %
                           (error.inName, error.inName, error.cellId,
                           error.objId, error.iObj, error.iMod,
                           error.masterName))
...
class File_SortRecords(Link):
    ...
    def Execute(self, event):
        ...
        if gui.dialog.ContinueQuery(self.window, message, 'mash.sortRecords.continue',
...
class File_StatsList(guiWxList):
    def __init__(self, parent, data):
        # --Columns
        self.cols = mosh.settings['mash.fileStats.cols']
        self.colNames = mosh.settings['mash.colNames']
        self.colWidths = mosh.settings['mash.fileStats.colWidths']
        self.colAligns = mosh.settings['mash.fileStats.colAligns']
        self.colReverse = mosh.settings['mash.fileStats.colReverse'].copy()
        # --Data/Items
        self.data = data
        self.sort = mosh.settings['mash.fileStats.sort']
        # --Links
        self.mainMenu = []
        self.itemMenu = []
        # --Parent init
...
class Installers_AutoAnneal(Link):
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        menuItem = wx.MenuItem(menu, self.id, _('Auto-Anneal'),
            kind=wx.ITEM_CHECK)
        menu.AppendItem(menuItem)
        menuItem.Check(mosh.settings['bash.installers.autoAnneal'])

    def Execute(self, event):
        """Handle selection."""
        mosh.settings['bash.installers.autoAnneal'] ^= True
...
class Installers_Enabled(Link):
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        menu.AppendItem(menuItem)
        menuItem.Check(mosh.settings['bash.installers.enabled'])

    def Execute(self, event):
        """Handle selection."""
        enabled = mosh.settings['bash.installers.enabled']
        message = _(
        ...
        if not enabled and not balt.askYes(self.gTank, fill(message, 80),
            self.title):
            return
        enabled = mosh.settings['bash.installers.enabled'] = not enabled
        if enabled:
...
class Installers_ConflictsReportShowsInactive(Link):
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        menuItem = wx.MenuItem(menu, self.id, _('Show Inactive Conflicts'),
            kind=wx.ITEM_CHECK)
        menu.AppendItem(menuItem)
        menuItem.Check(mosh.settings['bash.installers.conflictsReport.showInactive'])

    def Execute(self, event):
        """Handle selection."""
        mosh.settings['bash.installers.conflictsReport.showInactive'] ^= True
...
class Installers_ConflictsReportShowsLower(Link):
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        menuItem = wx.MenuItem(menu, self.id, _('Show Lower Conflicts'),
            kind=wx.ITEM_CHECK)
        menu.AppendItem(menuItem)
        menuItem.Check(mosh.settings['bash.installers.conflictsReport.showLower'])

    def Execute(self, event):
        """Handle selection."""
        mosh.settings['bash.installers.conflictsReport.showLower'] ^= True
        self.gTank.RefreshUI()
...
class Installers_AvoidOnStart(Link):
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        menuItem = wx.MenuItem(menu, self.id, _('Avoid at Startup'),
            kind=wx.ITEM_CHECK)
        menu.AppendItem(menuItem)
        menuItem.Check(mosh.settings['bash.installers.fastStart'])

    def Execute(self, event):
        """Handle selection."""
        mosh.settings['bash.installers.fastStart'] ^= True
...
class Installers_RemoveEmptyDirs(Link):
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        menu.AppendItem(menuItem)
        menuItem.Check(
            mosh.settings['bash.installers.removeEmptyDirs'])

    def Execute(self, event):
        """Handle selection."""
        mosh.settings['bash.installers.removeEmptyDirs'] ^= True
...
class Installers_SortActive(Link):
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        menuItem = wx.MenuItem(menu, self.id, _("Sort by Active"),
            kind=wx.ITEM_CHECK)
        menu.AppendItem(menuItem)
        menuItem.Check(mosh.settings['bash.installers.sortActive'])

    def Execute(self, event):
        mosh.settings['bash.installers.sortActive'] ^= True
...
class Installers_SortProjects(Link):
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        menuItem = wx.MenuItem(menu, self.id, _("Projects First"),
            kind=wx.ITEM_CHECK)
        menu.AppendItem(menuItem)
        menuItem.Check(mosh.settings['bash.installers.sortProjects'])

    def Execute(self, event):
        mosh.settings['bash.installers.sortProjects'] ^= True
        self.gTank.SortItems()
...
class Installers_SortStructure(Link):
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        menuItem = wx.MenuItem(menu, self.id, _("Sort by Structure"),
            kind=wx.ITEM_CHECK)
        menu.AppendItem(menuItem)
        menuItem.Check(mosh.settings['bash.installers.sortStructure'])

    def Execute(self, event):
        mosh.settings['bash.installers.sortStructure'] ^= True
        self.gTank.SortItems()
...
class Installer_HasExtraData(InstallerLink):
    ...
    def AppendToMenu(self, menu, window, data):
        Link.AppendToMenu(self, menu, window, data)
        menuItem = wx.MenuItem(menu, self.id, _('Has Extra Directories'),
            kind=wx.ITEM_CHECK)
        menu.AppendItem(menuItem)
        menuItem.Enable(self.isSingle())
        if self.isSingle():
...
class InstallerArchive_Unpack(InstallerLink):
    ...
    def Execute(self, event):
        ...
        result = balt.askText(self.gTank,
        ...
        if not project.s or project.cext in ('.rar', '.zip', '.7z'):
            balt.ShowWarning(self.gTank,
                _("%s is not a valid project name.") % result)
            return
        if self.data.dir.join(project).isfile():
            balt.ShowWarning(self.gTank, _("%s is a file.") % project.s)
            return
        if project in self.data:
            if not balt.askYes(self.gTank,
                    _("%s already exists. Overwrite it?") % project.s,
                self.title, False):
                return
        # --Copy to Build
...
class Mods_LoadListData(ListEditorData):
    ...
    def __init__(self, parent):
        """Initialize."""
        self.data = mosh.settings['mash.loadLists.data']
        # --GUI
        ListEditorData.__init__(self, parent)
        self.showRename = True
        self.showRemove = True
    ...
    def rename(self, oldName, newName):
        ...
        if len(newName) == 0 or len(newName) > 64:
            ...
            return False
        # --Rename
        mosh.settings.setChanged('mash.loadLists.data')
        self.data[newName] = self.data[oldName]
        del self.data[oldName]
        return newName
    ...
    def DoSave(self, event):
        ...
        if result == wx.ID_OK:
            ...
            if len(newItem) == 0 or len(newItem) > 64:
                gui.dialog.ErrorMessage(self.window,_('Load list name must be between 1 and 64 characters long.'))
            else:
...
class Mods_MorrowindIni(Link):
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        menu.AppendItem(menuItem)
        self.path = os.path.join(mosh.settings['mwDir'], 'Morrowind.ini')
...
class Mods_LockTimes(Link):
    ...
    def Execute(self, event):
        ...
        mosh.settings['mosh.modInfos.resetMTimes'] = mosh.modInfos.resetMTimes
        if mosh.modInfos.resetMTimes:
...
class Mods_Replacers(Link):
    ...
    def Execute(self, event):
        ...
        dialog = ListEditorDialog(self.window, -1, _('Replacers'), data,
            'checklist')
        dialog.ShowModal()
        dialog.Destroy()
...
class Mods_IniTweaks(Link):
    ...
    def Execute(self, event):
        ...
        if gui.dialog.ContinueQuery(self.window, message, 'mash.iniTweaks.continue',
            _('INI Tweaks')) != wx.ID_OK:
            return
        # --File dialog
        mitDir = os.path.join(mosh.modInfos.dir, 'Mits')
        if not os.path.exists(mitDir):
            mitDir = mosh.settings['mwDir']
        ...
        gui.dialog.InfoMessage(self.window, _('%s applied.') % (os.path.split(mitPath)[1],), _('INI Tweaks'))
...
class Mods_Mlox():
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        revertItem.Enable(self.HasMlox() and self.settingsKey in mosh.settings)
    ...
    def LaunchMlox(self, event):
        ...
        if os.path.exists('mlox.exe'):
            ...
            gui.dialog.ErrorMessage(self.window,
                _('Couldn\'t find mlox.exe to launch'))

        os.chdir(cwd)

    def MloxRevert(self, event):
        if self.settingsKey in mosh.settings:
            MloxSorter().update_mod_times(
                mosh.settings[self.settingsKey])
            del mosh.settings[self.settingsKey]
        else:
            gui.dialog.ErrorMessage(self.window,
                _('Cannot revert. Nothing to revert to'))

    def MloxSort(self, event):
        items = mosh.modInfos.keys()
        items.sort(key=lambda x: mosh.modInfos[x].mtime)

        mosh.settings['mash.ext.mlox.oldorder'] = items
...
class Mod_LabelsData(ListEditorData):
    ...
    def __init__(self, parent, strings):
        ...
        # --Key/type
        self.data = mosh.settings[self.setKey]
        # --GUI
        ListEditorData.__init__(self, parent)
        self.showAdd = True
        self.showRename = True
    ...
    def add(self):
        ...
        if newName in self.data:
            ...
            gui.dialog.ErrorMessage(self.parent,
                _('Name must be between 1 and 64 characters long.'))
            return False
        mosh.settings.setChanged(self.setKey)
...
class Mod_Labels:
    ...
    def __init__(self):
        """Initialize."""
        self.labels = mosh.settings[self.setKey]
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        wx.EVT_MENU_RANGE(window, self.idList.BASE, self.idList.MAX,
            self.DoList)
...
class Mod_CopyToEsmp(Link):
    ...
    def Execute(self, event):
        ...
        if os.path.exists(os.path.join(modsDir, newName)):
            result = gui.dialog.WarningMessage(self.window,
...
class Mod_Export_Dialogue(Link):
    ...
    def Execute(self, event):
        ...
        textDir = mosh.settings.get('mosh.workDir', mosh.settings['mwDir'])
        # --File dialog
        ...
        mosh.settings['mosh.workDir'] = textDir
        mosh.settings['mash.dialEdit.path'] = textPath
        # --Export
...
class Mod_Export_Scripts(Link):
    ...
    def Execute(self, event):
        ...
        (textDir, textName) = os.path.split(textPath)
        mosh.settings['mosh.workDir'] = textDir
        mosh.settings['mash.scriptEdit.path'] = textPath
        # --Export
...
class Mod_Import_Dialogue(Link):
    ...
    def Execute(self, event):
        """Handle menu selection."""
        fileName = self.data[0]
        textPath = mosh.settings.get('mash.dialEdit.path')
        if textPath:
            (textDir, textName) = os.path.split(textPath)
        else:
            textName = os.path.splitext(fileName)[0] + '_Dialogue.txt'
...
class Mod_Import_LCVSchedules(Link):
    ...
    def Execute(self, event):
        ...
        if gui.dialog.ContinueQuery(self.window, message, 'mash.schedules.import.continue',
            _('Import LCV Schedules...')) != wx.ID_OK:
            return
...
class Mod_Import_Scripts(Link):
    ...
    def Execute(self, event):
        """Handle menu selection."""
        fileName = self.data[0]
        textPath = mosh.settings.get('mash.scriptEdit.path')
        if textPath:
            (textDir, textName) = os.path.split(textPath)
        else:
            textName = os.path.splitext(fileName)[0] + '_Scripts.mws'
...
class Mod_Tes3cmd_Clean(Link):
    ...
    def OnDone(self):
        logDir = os.path.join(mosh.settings['mwDir'],
            'Data Files',
            'tes3cmd',
            'Logs')
    ...
    def Execute(self, event):
        ...
        try:
            ...
            if curFirst == 0:
                ...
                return
            table = self.window.data.table
            first = table.getItem(fileName, 'firstObjectIndex',
                random.randint(1001, 10001))
            dialog = wx.TextEntryDialog(self.window,
            ...
            if not (0 < first <= 100000):
                gui.dialog.ErrorMessage(self.window,
                    _("Object index must be an integer in range 1:100,000."))
                return
            if first == curFirst:
                gui.dialog.ErrorMessage(self.window,
                    _("New object index is same as old object index!"))
                return
            # --Renumber objects
...
class Mod_UpdatersData(ListEditorData):
    ...
    def add(self):
        ...
        # --File dialog
        dialog = wx.FileDialog(self.parent, _('Select previous version:'),
            modDir,
            '', wildcard, wx.OPEN)
        if dialog.ShowModal() != wx.ID_OK:
            dialog.Destroy()
        ...
        if fromDir.lower() != modDir.lower():
            gui.dialog.ErrorMessage(self.parent,
                _('Previous mod file must be located in Data Files directory.'))
            return None
        # --Old Refs
        oldInfo = mosh.modInfos[fromMod]
        oldRefs = oldInfo.extras.get('FileRefs')
        if not oldRefs:
            oldRefs = oldInfo.extras['FileRefs'] = mosh.FileRefs(oldInfo, True,
                True)
            oldRefs.refresh()
        # --New Refs
        ...
        if not newRefs:
            newRefs = newInfo.extras['FileRefs'] = mosh.FileRefs(newInfo, True,
                True)
            newRefs.refresh()
        # --Remap
        ...
        # --No object map to save?
            ...
            gui.dialog.InfoMessage(self.parent,
                _("No updater required for conversion from %s to %s.") %
                (fromMod, self.toMod))
            return None
...
class Saves_ProfilesData(ListEditorData):
    ...
    def rename(self, oldName, newName):
        ...
        if oldName == self.defaultName:
            gui.dialog.ErrorMessage(self.parent,
                self.defaultName + _(' cannot be renamed.'))
            return False
        ...
        if oldName == mosh.settings['mash.profile']:
            mosh.settings['mash.profile'] = newName
        return newName

    def remove(self, profile):
        """Removes load list."""
        # --Can't remove active or Default directory.
        if profile == mosh.settings['mash.profile']:
...
class Saves_Profiles:
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        # --Profiles
        items = self.GetItems()
        curProfile = mosh.settings.get('mash.profile', self.defaultName)
    ...
    def DoList(self, event):
        """Handle selection of label."""
        # --Profile Names
        arcProfile = mosh.settings.get('mash.profile', self.defaultName)
        ...
        try:
            ...
            mosh.settings['mash.profile'] = srcProfile
...
class Saves_MapGridLines(Link):
    ...
    def AppendToMenu(self, menu, window, data):
        ...
        menuItem = wx.MenuItem(menu, self.id, _('World Map Gridlines'),
            kind=wx.ITEM_CHECK)
        menu.AppendItem(menuItem)
        menuItem.Check(mosh.settings['mash.worldMap.gridLines'])

    def Execute(self, event):
        """Handle menu selection."""
        mosh.settings['mash.worldMap.gridLines'] = not mosh.settings[
...
class Save_MapNotes(Link):
    ...
    def Execute(self, event):
        ...
        log = mosh.LogFile(cStringIO.StringIO())
        try:
            fileRefs = mosh.FileRefs(fileInfo, True, True, log=log,
                progress=progress)
            fileRefs.refresh()
    ...
    def Execute(self, event):
        ...
        try:
            ...
            if count:
                fileRefs.removeOrphanContents()
                fileRefs.safeSave()
                gui.dialog.InfoMessage(self.window,
                    _("%d spawn points removed/reset.") % (count,))
            else:
...
class Save_Remove_DebrisCells(Link):
    ...
    def Execute(self, event):
        ...
        if gui.dialog.ContinueQuery(self.window, message,
            'mash.removeDebrisCells.continue',
            _('Remove Debris Cells')) != wx.ID_OK:
            return
        # --File Info
        fileName = self.data[0]
        fileInfo = self.window.data[fileName]
    ...
    def Execute(self, event):
        ...
        try:
            ...
            worldRefs.repairWorldMap(fileRefs,
                mosh.settings['mash.worldMap.gridLines'])
            fileRefs.safeSave()
            progress = progress.Destroy()
    ...
    def Execute(self, event):
        ...
        if not maPattern:
            balt.showError(self.window,
                _("Bad extension or file root: ") + pattern)
            return
...
class App_Morrowind(Link):
    ...
    def Execute(self, event):
        """Handle menu selection."""
        cwd = os.getcwd()
        os.chdir(mosh.settings['mwDir'])
        os.spawnl(os.P_NOWAIT, 'Morrowind.exe', 'Morrowind.exe')
        os.chdir(cwd)
        if mosh.settings.get('mash.autoQuit.on', False):
...
class AutoQuit_Button(Link):
    ...
    def SetState(self, state=None):
        ...
        if state == None:  # --Default
            state = mosh.settings.get('mash.autoQuit.on', False)
        elif state == -1:  # --Invert
            state = not mosh.settings.get('mash.autoQuit.on', False)
        mosh.settings['mash.autoQuit.on'] = state
...
class App_Help(Link):
    ...
    def Execute(self, event):
        ...
        # if not helpBrowser:
        HelpBrowser(globalvars.mashFrame, globalvars.images).Show()
        mosh.settings['mash.help.show'] = True
...
class App_Settings(Link):
    ...
    def Execute(self, event):
        ...
        if not globalvars.settingsWindow:
            globalvars.settingsWindow = SettingsWindow()
            globalvars.settingsWindow.SetSettings(mosh.settings, Inst=mosh.dirs["installers"].s)
            globalvars.settingsWindow.Show()
            mosh.settings['mash.settings.show'] = True
...
def InitSettings():
    ...
    mosh.settings.loadDefaults(conf.settingDefaults)
