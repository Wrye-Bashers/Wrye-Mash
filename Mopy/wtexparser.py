...
def getHtmlFromHeadings(headings):
    ...
    def getHtmlFromHeading(heading):
        html = '<a name="' + heading.title.replace(' ',
            '') + '"><strong>' + heading.title + '</strong><br>'
        for line in heading.getTextLines():
            html += '&nbsp;' * (line.level - 1) * 2 + getHtmlFromLine(
                line.text) + '<br>'
        return html
...
class Parser:
    ...
    def insert(self, currentNode, level, creator):
        ...
        # and recheck everything
            ...
            assert currentNode.parent != None, "At " + str(
                currentNode.level) + ", there was no parent when searching for level " + str(
                level - 1)
    ...
    def parseText(self, text):
        ...
        for r in result:
            ...
            for newResult in newResults:
                newResult.mergeWith(
                    r)  # copy properties down from this level to the one below
...
class TestHtml(unittest.TestCase):
    def testGenerate(self):
        ...
        self.assertEqual(
            '<p><strong id="The Name">The Name</strong><br>Some Text<br></p>',
            html)
